...
class MCPGateway:
    ...
    def execute_tool(
        ...
        if "jira_create_issue" in tool_name or "jira_update_issue" in tool_name:
            issue_key = params.get("issue_key", params.get("parent_key", "AEGIS-431"))
            result_payload = {
                "system": "Jira Cloud",
                "issue_key": f"{issue_key}-EXECUTED" if not str(issue_key).endswith("EXECUTED") else issue_key,
                "operation": "UPDATED" if "update" in tool_name else "CREATED",
                "message": f"Successfully executed {tool_name} on {issue_key} per human approval.",
            ...
            result_payload = {
                ...
                "repo": "acmefin/payments-core",
                "operation": "TAG_CREATED",
                "tag": params.get("tag", "v2.2.0-rc1"),
                "commit": "b4e19f2a",
                "timestamp": datetime.utcnow().isoformat(),
            }
